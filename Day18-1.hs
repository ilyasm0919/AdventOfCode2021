...
data Snailfish = Number Int | Pair Snailfish Snailfish
...
main = readFile "Input18.txt" >>= print . magnitude . foldl1 (\x y -> reduce $ Pair x y) . fmap (evalState parseSnailfish) . lines
